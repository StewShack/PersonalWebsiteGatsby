class NumericInputTest {
    ...
    inputNotNumber(value) {
        if (value.search(/</) >= 0) {
            let resultXssAttack = this.numericInputTestResults[12]
            resultXssAttack.Input = value
            return resultXssAttack
        }

        if (value.search(/--/gm) >= 0) {
            let resultSqlInjection = this.numericInputTestResults[13]
            resultSqlInjection.Input = value
            return resultSqlInjection
        }

        let resultNan = this.numericInputTestResults[3]
    ...
    testInput(value) {
        ...
        if (value.search(/^0[0-9]/gm) >= 0) {
            let result = this.numericInputTestResults[4]
            result.Input = value
            return result
        }

        if (value.search(/^0$/gm) >= 0) {
            let result = this.numericInputTestResults[5]
            result.Input = value
            return result
        }

        if (value.search(/^1$/gm) >= 0) {
            let result = this.numericInputTestResults[6]
            result.Input = value
            return result
        }

        if (value.search(/^100$/gm) >= 0) {
            let result = this.numericInputTestResults[7]
            result.Input = value
            return result
        }

        if (value.search(/^101$/gm) >= 0) {
            let result = this.numericInputTestResults[8]
            result.Input = value
            return result
        }

        if (!isFinite(value) || value.search(/\./gm) >= 0 || (value > 0 && value < lowerBound)) {
            let result = this.numericInputTestResults[9]
            result.Input = value
            return result
        }

        if (value < 0 || value < Number.MIN_VALUE) {
            let result = this.numericInputTestResults[10]
            result.Input = value
            return result
        }

        if (value > 101 || value > Number.MAX_VALUE) {
            let result = this.numericInputTestResults[11]
            result.Input = value
            return result
        }

        if (value > 1 && value < upperBound) {
            let result = this.numericInputTestResults[0]
            result.Input = value
            return result
        }
